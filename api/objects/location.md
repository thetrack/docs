...
**Location** - Это объект описывающий адрес или географические координаты точки. Location используется в API для описания каких либо объектов находящихся в пространстве. Например Location описывает место в которое осуществляется доставка для клиента.
...
{% method -%}
## Создание объект Location {#location-create}
...
### Возвращает
API возвращает объект Location с его идентификатором.
```javascript
{
